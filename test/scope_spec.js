...
describe("digest", function () {
  ...
  it("triggers chained watcher in the same digest", function () {
    scope.name = 'nahid';
    scope.$watch(function (scope) {
    ...
    // expect(scope.initial).toBe(undefined);
  ...
  it("has a $$phase field whose value is the current digest phase", function () {
    ...
    scope.phaseInListernerFunction = undefined;
    ...
    scope.$watch(function (scope) {
      ...
      scope.phaseInListernerFunction = scope.$$phase;
    });
    ...
    expect(scope.phaseInListernerFunction).toBe('$digest');
  ...
  it("catches exceptions in watch functions and continue", function () {
    scope.aValue = 'abc';
    scope.counter = 0;
    scope.$watch(function (scope) {
      throw "error";
...
describe("inheritance", function () {
  ...
  it("does not shadows members of parent scope's attribute", function () {
  ...
  it("can take some other scope as the parent", function () {
    ...
    child.counter = 0;
    child.$watch(function (scope) {
      scope.counter++;  //notice no return
    });

    prototypeParent.$digest(); //prototype $$children is empty
  ...
  it("is no longer digested when $destroy has been called", function () {
    ...
    parent.$digest();
    expect(child.counter).toBe(1);
    expect(watcherFnCalled).toBe(2);  // my assertion
...
describe("$watchCollection", function () {
  ...
  it("allows destroying of $watchCollection", function () {

    scope.counter = 0;
    scope.aValue = {a:1, b:2};

    var destroyWatchCollection = scope.$watchCollection(function (scope) {
      return scope.aValue;
    }, function (newVal, oldVal, scope) {
      scope.counter++;
    });
...
describe("Events", function () {
  ...
  _.forEach(['$emit', '$broadcast'], function (method) {
    ...
    it("calls the listeners registered for matching events on " + method, function () {
    ...
    it("passes an same event object to each listeners on " + method, function () {
    ...
    it("does not skp the next listener when removed on " + method, function () {
  ...
  it("propagates down scope hierarchy on $broadcast", function () {
  ...
  it("attaches currentScope on $broadcast", function () {
    ...
    //mine
    expect(evt.currentScope).toBe(null);
  ...
  //* -- chapter 9 expression and watches --*/
  ...
  it('accepts expressions for watch collection', function () {
    var theValue;
  ...
  it('accepts expressions in $eval with argument', function () {
    scope.aFunction = function(n) { return n; };

    expect(scope.$eval('aFunction(42)')).toBe(42);
  });
  ...
  // one time watch
  it('accepts one time watchers', function () {
    var theValue;
