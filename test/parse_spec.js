...
describe("parse", function () {
  ...
  // my own
  it("throws when number has more than one dot", function () {
    var fn = () => parse('.4.2');
    //expect(fn).toThrow();
  });
  ...
  it("can parse a scientific notation with a float negative exponents", function () {
    var fn = parse('4200e-2');
    expect(fn()).toBe(42);
  });

  it("can parse a scientific notation with a + sign", function () {
    var fn = parse('.42e+2');
    expect(fn()).toBe(42);
  });
  ...
  it("can parse in string in double quotes", function () {
    var fn = parse('"abc"');
    expect(fn()).toEqual('abc');
  });
  ...
  it("will parse an non empty object", function () {
    var fn = parse('{"a key":1, \'another-key\':2, "foo": [1,2]}');
    expect(fn()).toEqual({'a key': 1, 'another-key': 2, 'foo': [1, 2]});
  });

  it("will parse an object with identifier keys", function () {
    var fn = parse('{"a key":1, k:2, b:[2,3], c: {d:4}}');
    expect(fn()).toEqual({'a key': 1, k:2, b: [2,3], c: {d:4}});
  });
  ...
  it("parses a function call with a single number argument", function () {
    ...
    var scope = {aFunction: function(n) { return n; }};
    expect(fn(scope)).toBe(42);
  });
  ...
  it("parses a function call with a single identifier argument", function () {
    ...
    var scope = {n: 42, aFunction: function(arg) { return arg; }};
    expect(fn(scope)).toBe(42);
  });
  ...
  it("parses a function call with a single function call argument", function () {
    ...
    var scope = {argFn: _.constant(42), aFunction: function(arg) { return arg; }};
    expect(fn(scope)).toBe(42);
  });

  it("parses a function call with a multiple argument", function () {

    var fn = parse('aFunction(37, n, argFn())');
    var scope = {n: 3, argFn: _.constant(2), aFunction: function(a1, a2, a3) { return a1+a2+a3; }};
    expect(fn(scope)).toBe(42);
  });
  ...
  it("does not allow passing window as function argument", function () {
    ...
    var scope = {aFunction: function() {}, wnd: window };
  ...
  it("does not allow functions to return window", function () {
    ...
    var fn = parse('getWnd()');
    var scope = {getWnd: _.constant(window) };
  ...
  it("does not allow calling functions on DOM elements", function () {
    ...
    var scope = {el: document.documentElement };
  ...
  it("does not allow calling the aliased function constructor", function () {
    ...
    var scope = { fnConstructor: (function() {}).constructor};

    expect(function () { fn(scope); }).toThrow();
  });
  ...
  it("parses several multiplication", function () {
  ...
  it("parses multiplicative on a higher precedence than additives", function () {
  ...
  it("parses relational on a higher precedence than equality", function () {
  ...
  it("my own parses several statements edge case", function () {
    var fn = parse('a=1 b=2; c=3');
    var scope = {};
    fn(scope);
  ...
  /*-- new chapter Expressions & Watches */
  ...
  it('marks array literal', function () {
  ...
  it('marks integer constant', function () {
    var fn = parse('42');
  ...
  it('marks array constant when elements are constant', function () {
  ...
  // chapter 9 external assignment
  it('allows calling assign on identifier expression', function () {
  ...
  it('allows calling assign on member expression', function () {
